...
class RMSNorm(torch.nn.Module):
    """
    Root Mean Square Normlization

    RMS(x) = (1/d)sum_{i=1}{d}x_{i}^2

    1.为什么需要Weight参数
    """
    def __init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        # 防止除0的小数，通常为1e-6或1e-8
        self.eps = eps
        # 和输入特征维度一样的可学习放缩参数，初始化全为1
    ...
    def _rms_norm(self, x:torch.Tensor):
        ...
        均方根
        """
        rms_norm = x / (torch.sqrt(x.pow(x).mean(-1, keepdim=True)) + self.eps)
        # rms_norm = x / (x.square().mean(dim=-1, keepdim=True).sqrt() + self.eps)
        # rms_norm = x * torch.rsqrt(x.pow(2).mean(dim=-1, keepdim=True) + self.eps)
        return rms_norm

    def _layer_norm(self, x:torch.Tensor):
        pass

    def forward(self, x: torch.Tensor):
        input_dtype = x.dtype
        x = x.to(torch.float32)
        x = self._rms_norm(x).type_as(x)
