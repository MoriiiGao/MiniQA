...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            model.eval()
            moe_path = '_moe' if lm_config.use_moe else ''
            ckp = f'{args.save_dir}/DeciMind_full_sft_{lm_config.hidden_size}{moe_path}.pth'
...
if __name__ == "__main__":
    ...
    args.wandb_run_name = f"DeciMind-Full-SFT-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
